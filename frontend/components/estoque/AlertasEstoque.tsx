...
function StatusBadge({ status }: { status: "baixo" | "zerado" }) {
  if (status === "zerado") {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-500/15 text-erro border border-red-500/20">
        Zerado
      </span>
    );
  }
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-500/15 text-alerta border border-amber-500/20">
      Baixo
    </span>
  );
}
  ...
  if (loading) {
    return (
      <div className="bg-card shadow-elevacao border border-border rounded-xl p-5">
        <div className="h-5 w-40 bg-superficie-forte rounded animate-pulse mb-4" />
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-12 bg-superficie rounded-lg animate-pulse" />
          ))}
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-card shadow-elevacao border border-border rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-foreground">Alertas de Estoque</h3>
        </div>
    ...
    <div className="bg-card border border-amber-500/20 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <AlertTriangle className="h-5 w-5 text-alerta" />
          <h3 className="font-semibold text-foreground">Alertas de Estoque</h3>
          <span className="inline-flex items-center justify-center h-5 w-5 rounded-full bg-amber-500/20 text-alerta text-xs font-bold">
            {alertas.length}
          </span>
        </div>
        <Link
      ...
      <div className="space-y-2">
        {alertas.slice(0, 8).map((produto) => (
          <Link
            ...
            className="flex items-center justify-between p-3 rounded-lg bg-superficie hover:bg-superficie-forte transition-colors group"
          >
            ...
            <div className="flex items-center gap-3 flex-shrink-0 ml-3">
              <div className="text-right">
                <p className="text-sm font-semibold text-foreground">
                  ...
                  <span className="text-xs text-muted-suave">{produto.unidade}</span>
                </p>
                <p className="text-xs text-muted-suave">
