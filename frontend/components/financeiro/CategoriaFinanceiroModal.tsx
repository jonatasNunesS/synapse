      ...
      <div className="relative w-full max-w-2xl bg-card border border-border rounded-2xl shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-foreground">Categorias Financeiras</h2>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-superficie-forte transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row flex-1 min-h-0">
          {/* Form */}
          <div className="w-full lg:w-80 border-b lg:border-b-0 lg:border-r border-border p-6 flex-shrink-0">
            <h3 className="text-sm font-medium text-foreground-suave mb-4">
              {editando ? "Editar categoria" : "Nova categoria"}
            </h3>

            {serverError && (
              <div className="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-erro">
                {serverError}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-superficie border border-border rounded-lg text-sm text-foreground placeholder-slate-500 focus:outline-none focus:border-brand-500 transition-colors"
                />
              ...
              <div>
                ...
                {errors.cor && <p className="mt-1 text-xs text-erro">{errors.cor.message}</p>}
              </div>
              ...
              <div className="flex gap-2 pt-1">
                {editando && (
                  <button
                    type="button"
                    onClick={fecharForm}
                    className="flex-1 py-2 px-3 rounded-lg border border-border text-xs font-medium text-muted-foreground hover:bg-superficie transition-colors"
                  >
                    Cancelar
                  </button>
                )}
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="overflow-y-auto flex-1">
              {loading ? (
                <div className="flex items-center justify-center py-10 text-sm text-muted-suave">
                ...
                <div className="divide-y divide-border">
                  {categoriasFiltradas.map((cat) => (
                    <div key={cat.id} className="flex items-center gap-3 px-5 py-3 hover:bg-superficie transition-colors">
                      <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: cat.cor || "#6366f1" }} />
                      <div className="flex-1 min-w-0">
                        <span className="text-sm text-foreground truncate block">{cat.nome}</span>
                        <span className={`text-xs ${cat.tipo === "receita" ? "text-sucesso" : "text-erro"}`}>
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-muted-foreground hover:text-brand-accent hover:bg-brand-500/10 transition-colors"
                          title="Editar"
                        >
                          <Pencil className="h-3.5 w-3.5" />
                        </button>
                        {confirmandoDelete === cat.id ? (
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="px-2 py-1 rounded text-xs bg-red-500/20 text-erro hover:bg-red-500/30 transition-colors"
                            >
                              Confirmar
                            </button>
                            <button
