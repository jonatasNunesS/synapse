    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-xl border border-border bg-card shadow-xl">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border">
          <h2 className="text-sm font-semibold text-foreground">Trocar plano</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          <div>
            <p className="text-sm text-foreground-suave">
              Empresa: <span className="font-medium text-foreground">{empresaNome}</span>
            </p>
            <p className="text-xs text-muted-suave mt-0.5">
              Plano atual: <span className="text-foreground-suave">{planoAtual}</span>
            </p>
          </div>

          <div>
            <label className="text-xs text-muted-foreground block mb-1">Novo plano</label>
            <select
              value={planoNovo}
              onChange={(e) => setPlanoNovo(e.target.value as Plano)}
              className="w-full rounded-lg border border-border bg-secondary px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-amber-500"
            >
              {PLANOS.map((p) => (
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full resize-none rounded-lg border border-border bg-secondary px-3 py-2 text-sm text-foreground placeholder:text-muted-suave focus:outline-none focus:ring-1 focus:ring-amber-500"
            />
          </div>

          {/* Confirmação por digitação do nome */}
          <div className="rounded-lg border border-amber-500/30 bg-amber-950/30 p-3">
            <p className="text-xs text-alerta flex items-start gap-1.5 mb-2">
            ...
            <input
              ...
              className="w-full rounded-lg border border-border bg-secondary px-3 py-2 text-sm text-foreground placeholder:text-muted-suave focus:outline-none focus:ring-1 focus:ring-amber-500"
            />
          </div>
        </div>
